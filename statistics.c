...
struct Instruction frequency[TOTAL_INSTRUCTION] = {
    ...
    {"ADDI", 0},  {"SLLI", 0},  {"SLTI", 0}, {"SLTIU", 0},  {"XORI", 0},
    {"SRRI", 0},  {"SRLI", 0},  {"ORI", 0},  {"ANDI", 0},   {"AUIPC", 0},
    {"SB", 0},    {"SH", 0},    {"SW", 0},   {"ADD", 0},    {"SLL", 0},
    {"SLT", 0},   {"SLTU", 0},  {"XOR", 0},  {"SRL", 0},    {"OR", 0},
    {"AND", 0},   {"MUL", 0},   {"MULH", 0}, {"MULHSU", 0}, {"MULHU", 0},
    {"DIV", 0},   {"DIVU", 0},  {"REM", 0},  {"REMU", 0},   {"SUB", 0},
    {"SRA", 0},   {"LUI", 0},   {"BEQ", 0},  {"BNE", 0},    {"BLT", 0},
    {"BGE", 0},   {"BLTU", 0},  {"BGEU", 0}, {"JALR", 0},   {"JAL", 0},
    {"ECALL", 0}, {"EBREAK", 0}};
...
void top10_instruction()
{
    ...
    for (int i = 0; i < 10; i++)
        printf("\t %s:\t\t %d (%.2f %%)\n", frequency[i].name,
               frequency[i].count, frequency[i].count * 100.f / total);
}
...
void stats_information()
{
    ...
    printf("jump forward counter:%lu  (%.2f %%)\n", jump_forward,
           jump_forward * 100.0f / total);
    printf("jump backward counter:%lu (%.2f %%)\n", jump_backward,
           jump_backward * 100.0f / total);
    top10_instruction();
}
